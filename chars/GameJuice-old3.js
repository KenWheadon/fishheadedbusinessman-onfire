 ...
 * Features: 16:9 ratio, interactive HTML modal, headshot assets,
 * static hover styling, and physics effects.
 */
class GameJuice {
  constructor(containerId) {
    ...
    this.initModalEvents(); // New standard HTML event setup
    this.reset();
  ...
  initCharacters() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.characters.push({
        id: id,
        image: imgBody,
        headshot: imgHeadshot, // Store headshot
        ...
        dialogueIndex: 0,
        hovered: false // NEW state for static hover color
      });
    }
  }
  ...
  /**
   * Setup standard HTML button events for the custom modal
   */
  initModalEvents() {
   ...
   */
  handleCanvasClick(e) {
    // Block clicks if locked, winning, OR modal is already open
  ...
  /**
   * UPDATED to set character hovered state
   */
  handleCanvasMouseMove(e) {
    if (this.isLocked() || this.isWinning || this.isModalOpen) {
      this.canvas.style.cursor = 'default';
      // Reset hover states if board is inactive
    ...
    // Update internal hover state for drawing
  ...
  showModal(character) {
    ...
    this.pendingTarget = character; // Store who we might blow up

    // Set Headshot (fallback to C1 text if image loads slow/fails)
    if (character.headshot.complete && character.headshot.naturalWidth !== 0) {
      ...
      // Very basic fallback if headshot missing
      this.modalHeadshot.style.display = 'none';
    }

    this.modalTitle.textContent = `Target: Hero 0${character.id}`;
  ...
  next() {
    ...
    // Don't unlock if modal is open or winning
  ...
  reset() {
    ...
    this.characters.forEach(char => {
      ...
      char.hovered = false; // Reset hover
    });
  }

  win() {
    if (this.isModalOpen) return; // Prevent winning while modal is open
  ...
  // --- standard loop/particles/background remain same ---
  ...
  drawBackground() {
    ...
    this.characters.forEach(char => {
      if (char.alive) {
        ...
        // Dim spotlights if modal is open
  ...
  /**
   * REPLACED pulsing flashing logic with static color change on hover
   */
  drawCharacter(char) {
    this.ctx.save();
    ...
    // Dim cards if modal is open, unless this is the dying card
    ...
    if (!this.isLocked() && char.alive && !this.isWinning && !this.isModalOpen) {
      if (char.hovered) {
        // High intensity STATIC yellow/orange on hover
        this.ctx.strokeStyle = '#f9d423';
        this.ctx.lineWidth = 8;
        this.ctx.shadowColor = '#f9d423';
        this.ctx.shadowBlur = 20;
      } else {
        // Subtle static red border when just targetable
  ...
  drawInterfaceState() {
    ...
    // Hide interface text if modal is open
    ...
    if (this.isLocked()) {
      ...
      this.ctx.fillText('⚠️ DETONATION UNLOCKED: CLICK A HERO ⚠️', this.width / 2, 24);
    }
    this.ctx.restore();
  }
