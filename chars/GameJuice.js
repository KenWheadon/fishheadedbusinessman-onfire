 ...
 * Features: 16:9 ratio, interactive HTML split-modal, delayed detonation,
 * antic-jitter, floaty physics, and screen-shake mechanics.
 */
class GameJuice {
  constructor(containerId) {
    ...
    // Handle asynchronous explosion delay tasks cleanly
  ...
  initCharacters() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      imgBody.src = `images/char-${id}.webp`;

      const imgHeadshot = new Image();
      imgHeadshot.src = `images/headshot-${id}.jpg`;
      ...
      this.characters.push({
        ...
        dialogueIndex: 0,
        hovered: false,
        isWaitingForBoom: false // Dynamic "ticking countdown" state flag
      });
    }
  }
  ...
  // --- Custom Popups & Delayed Timing Engine ---
  ...
  /**
   * Triggers the delayed explosion sequencing
   */
  confirmExplosion() {
    if (this.pendingTarget) {
      const target = this.pendingTarget;
      
      // Lock the engine immediately so other cards cannot be clicked during countdown
      this.locked = true; 
  ...
  /**
   * Executes the physical explosion launch
   * Slowed down by ~50% to make the dynamic path highly visible.
   */
  explodeCharacter(target) {
    ...
    target.vx = direction * (Math.random() * 3.5 + 4.5); // (Reduced from 12-20)
    target.vy = -Math.random() * 6 - 8;                 // (Reduced from 15-35)
    target.va = direction * (Math.random() * 0.05 + 0.03); // Slower spin speed
    target.scale = 1.0;
  ...
  reset() {
    // Clear scheduled explosion tasks
    ...
    this.characters.forEach(char => {
      ...
      char.hovered = false; 
      char.isWaitingForBoom = false; // Clear timers
    });
  }
  ...
  spawnExplosion(x, y) {
    ...
    this.particles.push({ 
      ...
      vy: -5, // Slower upward speed
      scale: 1, 
      opacity: 1, 
      rotation: (Math.random() - 0.5) * 0.3, 
      life: 75, // Lasts longer on screen
      maxLife: 75 
    });

    // Elegant expanding shockwave ring
    this.particles.push({ 
      type: 'ring', 
      x: x, 
      y: y, 
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const speed = Math.random() * 8 + 3; // Reduced velocity
      this.particles.push({ 
        type: 'spark', 
        x: x, 
        y: y, 
  ...
  update() {
    this.characters.forEach(char => {
      if (char.alive) {
        ...
        // Slowed down physics loop for high visibility
        char.x += char.vx;
        char.y += char.vy;
        char.vy += 0.22; // Floaty gravity (Reduced from 0.85)
        char.angle += char.va; 
        char.scale = Math.max(0.1, char.scale - 0.0035); // Slow scale down (Reduced from 0.008)
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      p.opacity = progress;
      if (p.type === 'spark') { p.x += p.vx; p.y += p.vy; p.vy += 0.12; p.vx *= 0.98; } 
      else if (p.type === 'heart') { p.x += p.vx + Math.sin(this.time * p.driftSpeed) * 0.8; p.y += p.vy; p.scale = progress * 1.3; } 
      else if (p.type === 'text') { p.x += p.vx; p.y += p.vy; p.vy *= 0.96; p.scale = 1 + (1 - progress) * 1.2; } 
  ...
  drawCharacter(char) {
    ...
    // 1. Calculate anticipatory countdown shaking (Dramatic ticking Jitter)
    let jitterX = 0;
    let jitterY = 0;
    ...
    this.ctx.translate(char.x + jitterX, char.y + jitterY);
    ...
    // NEW: Ticking time bomb visual hazard outline right before they explode
    if (char.isWaitingForBoom) {
      this.ctx.strokeStyle = '#ff3b30';
      this.ctx.lineWidth = 10 + Math.sin(this.time * 0.6) * 4;
  ...
  drawInterfaceState() {
    ...
    if (isTargetCountingDown) {
      ...
      this.ctx.shadowColor = '#ff3b30'; this.ctx.shadowBlur = 12;
      this.ctx.font = '900 20px monospace';
      this.ctx.fillText('⚡ DETONATION INITIATED: STAND CLEAR ⚡', this.width / 2, 24);
