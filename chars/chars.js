/**
 * Chars - A modular, highly-polished HTML5 Canvas Game Component
 * Decoupled, self-contained, and highly portable.
 */
class Chars {
  constructor(config = {}) {
    // Configure virtual resolution coordinates
    this.width = config.width || 1280;
    this.height = config.height || 720;

    // Connect optionally to DOM references
    ...
    // Handle asynchronous explosion delay tasks
    ...
    // Internal virtual timeline tracker
    this.time = 0;
  }
  ...
  initCharacters() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // Safely access preloaded elements via a global AssetManager
  ...
  initModalEvents() {
    ...
    if (this.modalBtnConfirm) {
      this.modalBtnConfirm.addEventListener('click', this._onConfirmBound);
    }
    if (this.modalBtnCancel) {
      this.modalBtnCancel.addEventListener('click', this._onCancelBound);
    }
  }
  ...
  /**
   * Standard Input Interface Hooks
   * Resolves component hover dynamics and exposes cursor states back to host environment.
   */
  handleMouseMove(localX, localY) {
    ...
    const coords = { x: localX, y: localY };
  ...
  // --- Dynamic Popups & Delayed Timing Engine ---
  ...
  confirmExplosion() {
    if (this.pendingTarget) {
      ...
      this.locked = true; 
      target.isWaitingForBoom = true;

      // Delayed ticking countdown setup
      const delayMs = 500 + Math.random() * 1500;

      this.explosionTimeoutId = setTimeout(() => {
        target.isWaitingForBoom = false;
  ...
  /**
   * Delta Time (dt) Powered Physics Engine
   */
  update(dt) {
    // Normalizes physics updating relative to ~60 FPS standard frames
    ...
    if (this.isWinning) {
      const prevTimer = this.winTimer;
      this.winTimer -= frames;
      ...
      if (this.winTimer <= 0) {
        ...
        const interval = 8;
        const prevHeartCheck = Math.floor(prevTimer / interval);
        const currHeartCheck = Math.floor(this.winTimer / interval);
        if (prevHeartCheck !== currHeartCheck) {
          this.characters.forEach(char => { 
            if (char.alive) this.spawnHeart(char.x, char.y - 120); 
          });
        }
      }
    }
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'spark') { 
        ...
        p.x += (p.vx + Math.sin(this.time * p.driftSpeed) * 0.8) * frames; 
        p.y += p.vy * frames; 
  ...
  /**
   * Decoupled Drawing Loop Interface
   * Renders dynamically relative to host coordinates (x, y).
   */
  draw(ctx, x = 0, y = 0) {
    ctx.save();
    
    // Position component relative to bounding block placement
    ctx.translate(x, y);

    // Keep particles and assets cleanly clipped inside the module area
    ctx.beginPath();
    ...
    if (this.shakeIntensity > 0.1) {
      ...
      ctx.translate(dx, dy);
      this.shakeIntensity = this.isUltimateWin 
        ? Math.max(6, this.shakeIntensity * 0.97) 
    ...
    if (this.isUltimateWin) { 
      this.drawUltimateWinText(ctx); 
    } else { 
      this.drawInterfaceState(ctx); 
    }
  ...
  drawBackground(ctx) {
    ...
    for (let i = 0; i < perspectiveCount; i++) {
      ...
      ctx.beginPath(); 
      ctx.moveTo(startX, this.height * 0.75); 
    ...
    this.characters.forEach(char => {
      if (char.alive) {
        const glow = ctx.createRadialGradient(char.baseX, char.baseY - 40, 5, char.baseX, char.baseY - 40, 140);
        ...
        ctx.fillStyle = glow;
        ctx.beginPath(); 
        ctx.arc(char.baseX, char.baseY - 40, 140, 0, Math.PI * 2); 
        ctx.fill();
      }
    });
  }
  ...
  drawCharacter(ctx, char) {
    ...
    if (char.alive) {
      ctx.save(); 
      ctx.scale(1, 0.28);
      const shadowGrad = ctx.createRadialGradient(0, 0, 5, 0, 0, w / 2);
      shadowGrad.addColorStop(0, 'rgba(0, 0, 0, 0.5)'); 
      shadowGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
      ctx.fillStyle = shadowGrad; 
      ctx.beginPath(); 
      ctx.arc(0, 20, w / 2, 0, Math.PI * 2); 
      ctx.fill();
      ctx.restore();
    }
    ...
    if (!this.isLocked() && char.alive && !this.isWinning && !this.isModalOpen) {
      if (char.hovered) {
        ...
        ctx.strokeStyle = 'rgba(255, 78, 80, 0.6)';
        ctx.lineWidth = 4;
        ctx.shadowBlur = 0;
      }
      this.drawRoundedRect(ctx, drawX - 2, drawY - 2, w + 4, h + 4, 16);
      ctx.stroke();
      ctx.shadowBlur = 0; 
    }
    ...
    if (char.isWaitingForBoom) {
      ...
      ctx.lineWidth = 10 + Math.sin(this.time * 0.6) * 4;
      ctx.shadowColor = '#ff3b30';
      ctx.shadowBlur = 25;
      this.drawRoundedRect(ctx, drawX - 4, drawY - 4, w + 8, h + 8, 18);
      ctx.stroke();
      ctx.shadowBlur = 0;
    }
    ...
    if (char.image && char.image.complete && char.image.naturalWidth !== 0) {
      ...
      ctx.lineWidth = 4;
      this.drawRoundedRect(ctx, drawX, drawY, w, h, 14); 
      ctx.fill(); 
      ctx.stroke();
      
      ctx.fillStyle = `hsl(${char.id * 72}, 70%, 55%)`; 
      this.drawRoundedRect(ctx, drawX + 10, drawY + 10, w - 20, 36, 8); 
      ctx.fill();
      
      ctx.fillStyle = '#0f0f18'; 
      ctx.fillRect(drawX + 10, drawY + 54, w - 20, 114);
      
      ctx.fillStyle = '#fff'; 
      ctx.font = 'bold 38px sans-serif'; 
      ctx.textAlign = 'center'; 
      ctx.textBaseline = 'middle'; 
      ctx.fillText(`C${char.id}`, 0, drawY + 110);
      
      ctx.fillStyle = '#ffffff'; 
      ctx.font = '900 15px monospace'; 
      ctx.fillText(`HERO 0${char.id}`, 0, drawY + 28);
      
      ctx.fillStyle = '#656585'; 
      ctx.font = '11px sans-serif'; 
      ctx.fillText(`STABLE READY`, 0, drawY + 200);
    }
  ...
  drawInterfaceState(ctx) {
    ...
    const isTargetCountingDown = this.characters.some(c => c.isWaitingForBoom);

    if (isTargetCountingDown) {
      ctx.fillStyle = '#ff3b30';
      ctx.shadowColor = '#ff3b30'; 
      ctx.shadowBlur = 12;
      ctx.font = '900 20px monospace';
      ctx.fillText('⚡ DETONATION INITIATED: STAND CLEAR ⚡', this.width / 2, 24);
    } else if (this.isLocked()) {
      ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
      ctx.font = 'bold 16px monospace';
      ctx.fillText('STAGE SECURED / CLICK next() TO INITIATE', this.width / 2, 24);
    } else {
      ctx.fillStyle = '#ff4e50';
      ctx.shadowColor = '#ff4e50'; 
      ctx.shadowBlur = 8;
      ctx.font = '900 18px monospace';
      ctx.fillText('⚠️ DETONATION UNLOCKED: CHOOSE A TARGET HERO ⚠️', this.width / 2, 24);
    }
    ctx.restore();
  }
  ...
  drawParticles(ctx) {
    this.particles.forEach(p => {
      ...
      if (p.type === 'spark') { 
        ...
        ctx.font = `${p.scale * 36}px sans-serif`; 
        ctx.textAlign = 'center'; 
        ctx.textBaseline = 'middle'; 
        ...
        const textGrad = ctx.createLinearGradient(-40, -15, 40, 15); 
        ...
        ctx.strokeStyle = `rgba(255, 90, 0, ${p.opacity})`; 
        ctx.lineWidth = 8 * p.opacity; 
        ctx.beginPath(); 
  ...
  drawUltimateWinText(ctx) {
    ...
    ctx.strokeStyle = '#000000'; 
    ctx.lineWidth = 16; 
    ctx.strokeText('ULTIMATE WINNER!', 0, 0);
    
    const rotationGrad = ctx.createLinearGradient(-250, 0, 250, 0); 
    ...
    ctx.fillStyle = rotationGrad; 
    ctx.fillText('ULTIMATE WINNER!', 0, 0);
  ...
  destroy() {
    ...
    if (this.modalBtnConfirm && this._onConfirmBound) {
      this.modalBtnConfirm.removeEventListener('click', this._onConfirmBound);
    }
    if (this.modalBtnCancel && this._onCancelBound) {
      this.modalBtnCancel.removeEventListener('click', this._onCancelBound);
    }
  }
}