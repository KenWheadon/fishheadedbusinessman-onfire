...
class GameLoader {
  ...
  start() {
    ...
    this.statusText.textContent = 'Loading Game Resources...';
    
    this.runUpdateLoop();
  }

  /**
   * Sets the target loader percentage (forwards or backwards)
   */
  setProgress(percent) {
    ...
    if (this.isStarted) {
      if (this.targetProgress === 100) {
        ...
        this.statusText.textContent = `Loading... ${Math.round(this.targetProgress)}%`;
      }
    }
  }
   ...
   */
  runUpdateLoop() {
    const ease = 0.08; // Velocity multiplier. Lower = smoother/slower, Higher = snappier.
    
    const update = () => {
      // Calculate delta step
      const diff = this.targetProgress - this.currentProgress;

      // Update current progress incrementally
      ...
      // Render updated width onto the DOM
      this.barFill.style.width = `${this.currentProgress}%`;

      // Keep running the loop if there's visually noticeable movement to process
      if (this.isStarted) {
