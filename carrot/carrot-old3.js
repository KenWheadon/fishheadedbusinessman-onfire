class CarrotCutter {
  constructor(container, width, height) {
    ...
    // Physics Tuning (Bouncier, faster, high-impact)
    this.gravity = 0.5;            // Slightly heavier gravity to pull things down after high jumps
    this.bounce = 0.7;             // Bouncier collisions
    this.friction = 0.99;          // Slide further on the floor
  ...
  cut() {
    ...
    // --- EXPLOSIVE UPWARD POP MATH ---
    // Math.sin(carrot.angle) is negative, pointing upwards. 
    // We add a massive vertical boost (negative Y) to catapult it skyward.
    const basePower = 12 + Math.random() * 5;
    const extremeUpwardBlast = -16 - Math.random() * 8; // Massive vertical force vector

    const vx = Math.cos(carrot.angle) * basePower + (Math.random() * 4 - 2);
    const vy = Math.sin(carrot.angle) * basePower + extremeUpwardBlast;
    ...
    this.flyingTops.push({
      ...
      vy: vy,
      angle: carrot.angle,
      va: (Math.random() > 0.5 ? 1 : -1) * (0.35 + Math.random() * 0.4), // Wild high-speed spinning!
      r: this.carrotWidth / 2, 
      length: topPartLength
    });

    // --- UPWARD PARTICLE FOUNTAIN ---
    const particleCount = 45; // Denser cloud
    for (let i = 0; i < particleCount; i++) {
      // Force particles to shoot straight up (-Math.PI / 2) with a small spreading angle
      const pAngle = -Math.PI / 2 + (Math.random() - 0.5) * 1.4; 
      const pSpeed = 8 + Math.random() * 14;      
      this.particles.push({
        x: midX,
        y: midY,
        vx: Math.cos(pAngle) * pSpeed + (Math.random() - 0.5) * 3,
        vy: Math.sin(pAngle) * pSpeed - (5 + Math.random() * 6), // Heavy upward push
        size: 3 + Math.random() * 8,
        rotation: Math.random() * Math.PI * 2,
        vRot: (Math.random() - 0.5) * 0.7,
        color: `hsl(${14 + Math.random() * 15}, 100%, ${46 + Math.random() * 15}%)`,
        alpha: 1.0,
        decay: 0.009 + Math.random() * 0.012 // Particles last slightly longer in the air
      });
    }
  }
  ...
  checkSegmentCollision(top, A, B) {
    ...
    if (distSq < minDist * minDist) {
      ...
      if (dot < 0) {
        ...
        top.va += (top.vx * ny - top.vy * nx) * 0.05; // Extra spin transfer
      }
    }
  }
  ...
  update() {
    ...
    for (let top of this.flyingTops) {
      ...
      // Screen Ceiling (Violent bounce downwards!)
      if (top.y < top.r) {
        top.y = top.r;
        top.vy = -top.vy * this.bounce;
        top.va *= -0.8; // Reverse spin slightly on impact
      }
  ...
  render() {
    ...
    // Apply Screen Shake transformation (Biased vertically for heavy impact feeling)
    if (this.screenShake > 0) {
      const dx = (Math.random() - 0.5) * (this.screenShake * 0.5);
      const dy = (Math.random() - 0.5) * this.screenShake; // Heavy vertical shake
